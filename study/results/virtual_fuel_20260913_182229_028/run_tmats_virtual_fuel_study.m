function out=run_tmats_virtual_fuel_study(out,indices,caseIndices)
...
if nargin<1
 build_tmats_virtual_fuel_model;
 out=fullfile(root,'results',['virtual_fuel_' datestr(now,'yyyymmdd_HHMMSS_FFF')]);mkdir(out);
end
if worker || benchWorker
 cacheId=0;if worker,cacheId=indices(1);else,cacheId=20+caseIndices(1);end
 cache=fullfile(root,'tmp',sprintf('vfc%02d',cacheId));if ~exist(cache,'dir'),mkdir(cache);end
 Simulink.fileGenControl('set','CacheFolder',cache,'CodeGenFolder',cache,'createDir',true);
end
if ~worker && ~benchWorker,fid=fopen(fullfile(root,'tmp','virtual_fuel_directory.txt'),'w');fprintf(fid,'%s',out);fclose(fid);end
m='GasTurbine_Dyn_Template_VirtualFuel';load_system(m);cleanup=onCleanup(@()close_system(m,0));
...
grid=[.25 5;.5 10;1 10;1 20;2 20;2 40;4 40;4 80;8 80;8 160;16 160;16 320; ...
 .5 40;1 40;1 80;2 80];
...
for c=caseIndices
 for enabled=[false true]
  ...
  method='PI';if enabled,method='VirtualFuel';end
...
sources={'tmats_virtual_fuel_setup.m','tmats_virtual_fuel_sfun.m','build_tmats_virtual_fuel_model.m','run_tmats_virtual_fuel_study.m'};
...
fprintf('VIRTUAL_FUEL_COMPLETE %s Kp=%g Ki=%g\n',out,gains);
 ...
 function [d,met]=runone(rpm,fraction,ramp,gain,enabled)
  [MWS,DOB,PTO,VF]=tmats_virtual_fuel_setup(rpm,fraction,ramp*[.3 1.5 3],gain,enabled);
  ...
  if enabled
   active=d.time_s>=VF.start;assert(max(abs(d.VF_unsaturated(active)-VF.Kp*d.VF_error(active)-d.VF_integral(active)))<1e-9);
