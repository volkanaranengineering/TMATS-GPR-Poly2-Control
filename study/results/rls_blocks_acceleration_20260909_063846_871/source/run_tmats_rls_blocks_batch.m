try
    ...
    if ~exist(fullfile(root,[mdl '.mdl']),'file'), build_tmats_rls_blocks_model(); end
    ...
    raw=sim(input);
    outDir=fullfile(root,'results',['rls_blocks_acceleration_' datestr(now,'yyyymmdd_HHMMSS_FFF')]);
    mkdir(outDir);
    save(fullfile(outDir,'rls_run.mat'),'raw','MWS','RLS','-v7.3');
    fid=fopen(fullfile(root,'tmp','rls_blocks_directory.txt'),'w'); fwrite(fid,outDir,'char'); fclose(fid);
