function out=run_tmats_virtual_fuel_uqup_study(out,caseIndices)
...
if nargin<1
 build_tmats_virtual_fuel_uqup_model;
 out=fullfile(root,'results',['virtual_fuel_uqup_' datestr(now,'yyyymmdd_HHMMSS_FFF')]);mkdir(out);
end
if ~worker,fid=fopen(fullfile(root,'tmp','virtual_fuel_uqup_directory.txt'),'w');fprintf(fid,'%s',out);fclose(fid);end
if worker
 cache=fullfile(root,'tmp',sprintf('vfuqup%02d',caseIndices(1)));if ~exist(cache,'dir'),mkdir(cache);end
 Simulink.fileGenControl('set','CacheFolder',cache,'CodeGenFolder',cache,'createDir',true);
end
[~,~,~,cal]=tmats_virtual_fuel_uqup_setup;
sigmaReference=cal.sigmaReference;
factor=tmats_uncertainty_increasing_gain_factor([0 1 2 3 6 Inf]*sigmaReference,zeros(1,6),sigmaReference);
assert(max(abs(factor-[1 1 1.5 2 2 2]))<1e-14);
assert(tmats_uncertainty_increasing_gain_factor(0,3*sigmaReference,sigmaReference)==factor(4));
if ~worker,save(fullfile(out,'configuration.mat'),'gains','sigmaReference','fixed');end
m='GasTurbine_Dyn_Template_VirtualFuelUQUp';load_system(m);cleanup=onCleanup(@()close_system(m,0));
...
for c=caseIndices
 ...
 label=sprintf('%drpm_%02dpct_%s_VirtualFuelUQUp',cases(c,1),round(100*cases(c,2)),shape);
...
if ~worker
 sources={'tmats_virtual_fuel_uqup_setup.m','tmats_virtual_fuel_uqup_sfun.m','tmats_uncertainty_increasing_gain_factor.m','build_tmats_virtual_fuel_uqup_model.m','run_tmats_virtual_fuel_uqup_study.m'};
...
fprintf('UNCERTAINTY_STUDY_COMPLETE %s sigmaReference=%.12g\n',out,sigmaReference);
 function [d,met]=runone(rpm,fraction,ramp,gain,enabled)
  [MWS,DOB,PTO,VF]=tmats_virtual_fuel_uqup_setup(rpm,fraction,ramp*[.3 1.5 3],gain,enabled);
  ...
  method='PI';if enabled,method='VirtualFuelUQUp';end
