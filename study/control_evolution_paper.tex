...
\begin{titlepage}
\color{navy}\sffamily
{\small T-MATS CONTROLLER DEVELOPMENT / LONG-FORM RESEARCH SYNTHESIS}\par\vspace{17mm}
...
Document status & Simulation research synthesis; not hardware validation
\end{tabular}
\vfill
...
$T,P$ & Actual inlet temperature and pressure at the model query & K, kPa\\
...
Numerical acceptance, compressor-map range, and learned-model coverage are separate questions. The monitored corrected-speed coordinate is $N_c=N_s/(10000\sqrt{T/288.15})$, with a working map range $[0.5,1.05]$. A simulation can converge while leaving that range. Likewise, a GP query can lie in its training rectangle while leaving the joint training hull. The paper therefore reports these layers separately rather than treating convergence as evidence of physical or statistical validity.
...
\begin{equation}
\gamma[k]=\mathbf1\!\left[(u^\star<u_{\max}\ \lor\ e_v<0)
\land(u^\star>u_{\min}\ \lor\ e_v>0)\right].
\end{equation}
Then $I[k+1]=I[k]+\gamma[k]T_sK_i e_v[k]$. This conditional integration suppresses increments that drive the command farther into saturation, while permitting unwinding. It is not back-calculation. Integrator windup is a classical implementation concern~\cite{windup}; the exact gate here is established by the controller source.
...
Fallback substitutes $e_N^g=r-N_s$ and $a^g=\hat a$ if either derivative is at or below $10^{-5}$ in its own physical units, if the corrected compressor speed leaves $[0.5,1.05]$, or if
\begin{equation}
...
Independent trace replay checks all 35 accepted tuning/comparator trajectories. Recomputed RMSE and ringing agree with the stored summaries; maximum proportional-term and command reconstruction errors are approximately $2.54\times10^{-14}$ and $3.31\times10^{-14}$ lbm/s, and the integral recurrence error is about $1.12\times10^{-15}$ lbm/s (D20). These checks validate arithmetic and logging consistency, not model adequacy. The deployable study model is \path{GasTurbine_Poly2_9250_Tuned.mdl}; the earlier defaults remain preserved.
...
Fuel excess variation distinguishes reversals from monotonic fuel changes but depends on the selected window and sample interval. It can increase through high-frequency ripple or a few larger reversals, which have different actuator implications. During moving ambient conditions, some reversals may be necessary. No fuel-consumption benefit should be inferred from a lower reversal metric: total fuel mass would require $\int u\,dt$, which is a different quantity. A future actuator-oriented evaluation could add spectral energy, cycle counting, or bandwidth-weighted command variation, but these were not measured here.
...
The assembled LaTeX source is \path{control_evolution_paper.tex}. Tables are generated from the archived CSV files by \path{build_control_evolution_paper.py}. New early scheduling figures are generated by \path{plot_control_evolution.m}; other figures are copied from the finalized stage-specific reports and hashed. Compile the assembled source at least twice with \path{pdflatex} to resolve chapter, table, figure, and bibliography references. No shell escape or network access is required for LaTeX compilation.
