function tmats_environment_vf_sfun(b)
...
% residual integral, FF+PI unsaturated command, outside training rectangle flag.
b.NumDialogPrms=1;b.NumInputPorts=6;b.NumOutputPorts=8;
...
temperature=b.InputPort(5).Data;pressure=b.InputPort(6).Data;X=[r ar temperature pressure;n a temperature pressure];z=bsxfun(@rdivide,bsxfun(@rdivide,bsxfun(@minus,X,f.model.inputMean),f.model.inputScale),f.model.lengthScale');
fuel=zeros(2,1);
...
e=fuel(1)-fuel(2);u=f.feedforward*fuel(1)+f.Kp*e+b.Dwork(3).Data;
...
if b.CurrentTime<f.start || ~f.enabled
 b.Dwork(3).Data=b.InputPort(4).Data-f.feedforward*v(2)-f.Kp*v(4);
else
