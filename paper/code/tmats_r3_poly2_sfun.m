function tmats_r3_poly2_sfun(b)
% Three remedy architectures; preserve original eight outputs plus diagnostics.
b.NumDialogPrms=1;b.NumInputPorts=6;b.NumOutputPorts=13;
...
X=[r 0 temp pressure;n a temp pressure;n 0 temp pressure];
if f.mode==3,[mu,grad]=tmats_poly2_mean_gradient(f,X);else,mu=tmats_poly2_mean_gradient(f,X);grad=zeros(3,4);end
...
u=mu(1)+pterm+b.Dwork(3).Data;command=max(f.fuelMin,min(f.fuelMax,u));
...
v=[command;mu(1:2);ev;a;b.Dwork(3).Data;u;double(outside);ei;pterm;gn;ga;fallback];
end
function outputs(b)
...
if b.CurrentTime<f.start,b.Dwork(3).Data=b.InputPort(4).Data-v(2)-pterm;
